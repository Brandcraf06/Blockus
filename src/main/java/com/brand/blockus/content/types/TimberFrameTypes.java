...
public class TimberFrameTypes {
    ...
    public TimberFrameTypes(Block base, boolean fireproof) {
        ...
        this.lattice = BlocksRegistration.register(type + "_lattice", new PaneBlock(FabricBlockSettings.copy(base)));
        this.grate = BlocksRegistration.register(type + "_grate", new GrateBlock(FabricBlockSettings.copyOf(base).nonOpaque().allowsSpawning(Blocks::never).solidBlock(Blocks::never).suffocates(Blocks::never).blockVision(Blocks::never)));
