...
public class BlockusWorldgenProvider extends FabricDynamicRegistryProvider {
    ...
    protected void configure(RegistryWrapper.WrapperLookup registries, FabricDynamicRegistryProvider.Entries entries) {
        entries.add(registries.getOrThrow(RegistryKeys.CONFIGURED_FEATURE), LIMESTONE);
        entries.add(registries.getOrThrow(RegistryKeys.PLACED_FEATURE), PLACED_LIMESTONE_UPPER);
        entries.add(registries.getOrThrow(RegistryKeys.PLACED_FEATURE), PLACED_LIMESTONE_LOWER);

        entries.add(registries.getOrThrow(RegistryKeys.CONFIGURED_FEATURE), MARBLE);
        entries.add(registries.getOrThrow(RegistryKeys.PLACED_FEATURE), PLACED_MARBLE);

        entries.add(registries.getOrThrow(RegistryKeys.CONFIGURED_FEATURE), BLUESTONE);
        entries.add(registries.getOrThrow(RegistryKeys.PLACED_FEATURE), PLACED_BLUESTONE);

        entries.add(registries.getOrThrow(RegistryKeys.CONFIGURED_FEATURE), VIRIDITE);
        entries.add(registries.getOrThrow(RegistryKeys.CONFIGURED_FEATURE), VIRIDITE_EXTRA);

        entries.add(registries.getOrThrow(RegistryKeys.PLACED_FEATURE), PLACED_VIRIDITE);
        entries.add(registries.getOrThrow(RegistryKeys.PLACED_FEATURE), PLACED_VIRIDITE_EXTRA);

        entries.add(registries.getOrThrow(RegistryKeys.CONFIGURED_FEATURE), WHITE_OAK);
        entries.add(registries.getOrThrow(RegistryKeys.CONFIGURED_FEATURE), WHITE_OAK_LEAF_LITTER);
        entries.add(registries.getOrThrow(RegistryKeys.PLACED_FEATURE), PLACED_WHITE_OAK);
        entries.add(registries.getOrThrow(RegistryKeys.PLACED_FEATURE), PLACED_WHITE_OAK_RARE);
        entries.add(registries.getOrThrow(RegistryKeys.PLACED_FEATURE), WHITE_OAK_CHECKED);
    ...
    public static void bootstrapPlacedFeatures(Registerable<PlacedFeature> registerable) {
        ...
        // white oak
        RegistryEntry<ConfiguredFeature<?, ?>> whiteOakLeafLitter = configuredFeatures.getOrThrow(WHITE_OAK_LEAF_LITTER);
        PlacedFeatures.register(registerable, PLACED_WHITE_OAK, whiteOakLeafLitter, VegetationPlacedFeatures.treeModifiersWithWouldSurvive(PlacedFeatures.createCountExtraModifier(0, 0.04F, 1), BlockusBlocks.WHITE_OAK_SAPLING));
        PlacedFeatures.register(registerable, PLACED_WHITE_OAK_RARE, whiteOakLeafLitter, VegetationPlacedFeatures.treeModifiersWithWouldSurvive(PlacedFeatures.createCountExtraModifier(0, 0.0125F, 1), BlockusBlocks.WHITE_OAK_SAPLING));
