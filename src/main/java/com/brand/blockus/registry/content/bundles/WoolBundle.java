...
public record WoolBundle(
    ...
    public static <Id> WoolBundle register(ColorCollection<Id> ids, ColorCollection<Id> idsStairs, ColorCollection<Id> idsSlab, ColorCollection<Id> idsCarpet, TriFunction<Id, Function<BlockBehaviour.Properties, Block>, BlockBehaviour.Properties, Block> register) {
        ColorBlockItemCollection block = register(ids, register, (var0, p) -> new Block(p), BlockFactory.copyDyedBlocks(Blocks.WOOL), NumberProviders.COOKING_TIME_WOOL);
        WoolBundle bundle = new WoolBundle(block,
            register(idsStairs, register, (color, p) -> new StairBlock(block.blocks().pick(color).defaultBlockState(), p), BlockFactory.copyDyedBlocks(block.blocks()), NumberProviders.COOKING_TIME_WOOL),
            register(idsSlab, register, (var0, p) -> new SlabBlock(p), BlockFactory.copyDyedBlocks(block.blocks()), NumberProviders.COOKING_TIME_WOOL_SLABS),
            register(idsCarpet, register, WoolCarpetBlock::new, BlockFactory.copyDyedBlocks(Blocks.CARPET), NumberProviders.COOKING_TIME_WOOL_CARPETS)
        );
        LIST.add(bundle);
        return bundle;
    }
