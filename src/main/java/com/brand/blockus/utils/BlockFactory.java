...
public class BlockFactory {
    ...
    public static Block createStairs(String baseid, Block base) {
        ...
        if (BlockChecker.isAmethyst(baseid)) {
            return register(stairsId, new AmethystStairsBlock(base.getDefaultState(), AbstractBlock.Settings.copy(base)));
        } else if (BlockChecker.isRedstone(baseid)) {
            return register(stairsId, new RedstoneStairsBlock(base.getDefaultState(), AbstractBlock.Settings.copy(base)));
        } else {
            return register(stairsId, new StairsBlock(base.getDefaultState(), AbstractBlock.Settings.copy(base)));
        }
    }
    ...
    public static Block createWall(String baseid, Block base) {
        ...
        if (BlockChecker.isAmethyst(baseid)) {
            return register(wallId, new AmethystWallBlock(AbstractBlock.Settings.copy(base)));
        } else if (BlockChecker.isRedstone(baseid)) {
            return register(wallId, new RedstoneWallBlock(AbstractBlock.Settings.copy(base)));
        } else {
            return register(wallId, new WallBlock(AbstractBlock.Settings.copy(base)));
        }
    }
    ...
    public static Block registerSmallHedge(String id, Block base) {
        return register(id, new SmallHedgeBlock(AbstractBlock.Settings.copy(base).allowsSpawning(BlockFactory::canSpawnOnLeaves).suffocates(BlockFactory::never).blockVision(BlockFactory::never)));
    }
    ...
    public static Block createNeonBlock(DyeColor color) {
        return new Block(AbstractBlock.Settings.create().mapColor(color).luminance((state) -> 3).sounds(BlockSoundGroup.GLASS).strength(0.5f, 0.5f).emissiveLighting(BlockFactory::always).allowsSpawning(BlockFactory::never));
    }

    public static RedstoneLampBlock createRedstoneLamp() {
        return new RedstoneLampBlock(AbstractBlock.Settings.copy(Blocks.REDSTONE_LAMP).allowsSpawning(BlockFactory::always));
    }

    public static Block createLitRedstoneLamp() {
        return new Block(AbstractBlock.Settings.copy(Blocks.REDSTONE_LAMP).luminance((state) -> 15).allowsSpawning(BlockFactory::always));
    }

    public static Block createFuturneoBlock(MapColor color) {
        return new Block(AbstractBlock.Settings.create().mapColor(color).luminance((state) -> 15).strength(0.5f, 0.5f).sounds(BlockSoundGroup.GLASS).allowsSpawning(BlockFactory::always));
    }


    // Glass
    public static StainedGlassBlock createStainedGlass(DyeColor color, Block base) {
        return new StainedGlassBlock(color, AbstractBlock.Settings.copy(base).allowsSpawning(BlockFactory::never).solidBlock(BlockFactory::never).suffocates(BlockFactory::never).blockVision(BlockFactory::never));
    }
